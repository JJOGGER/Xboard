        ...
        fc.asyncProperty(
          ...
          async (initialLocale, targetLocale) => {
            ...
            const initialTranslations = {
              ...
              stats: i18n.global.t('dashboard.stats'),
            };
            ...
            expect(updatedTranslations.stats).toBe(mockMessages[targetLocale].dashboard.stats);
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          async (selectedLocale) => {
            ...
            // Simulate saving locale preference
            localStorage.setItem('locale', selectedLocale);

            // Verify it was saved
            expect(localStorage.getItem('locale')).toBe(selectedLocale);
        ...
        fc.asyncProperty(
          ...
          async (localeChanges) => {
            ...
            for (const locale of localeChanges) {
              ...
              const restoredLocale = localStorage.getItem('locale');
            ...
            expect(localStorage.getItem('locale')).toBe(finalLocale);
          }
        ),
        { numRuns: 50 }
      );
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          async (firstLocale, secondLocale) => {
            ...
            expect(localStorage.getItem('locale')).toBe(firstLocale);

            // Save second locale (should overwrite)
            localStorage.setItem('locale', secondLocale);
            expect(localStorage.getItem('locale')).toBe(secondLocale);
